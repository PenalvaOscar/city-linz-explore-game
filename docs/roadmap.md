...
- Google Maps on Android needs a key even in Expo Go (SDK 55+ rejects the embedded one, issue #4) → Android uses a Leaflet WebView (CARTO Voyager tiles, issue #7); needs network for Leaflet from the CDN and for tiles.
